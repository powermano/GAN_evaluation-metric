...
# 交叉熵损失函数
# sigmoid_cross_entropy_with_logits函数的输入是logits和targets，logits就是神经网络模型中的
# W
# X矩阵，且不需要经过Sigmoid激活函数。而targets的shape和logits相同，即正确的标注值。若令x = logits、 z = labels，那么该函数的表达式为z - log(sigmoid(x)) + (
#         1 - z) * -log(1 - sigmoid(x))
...
for it in range(20000):
    ...
    if it % 2000 == 0:
        ...
        fig = plot(samples)
        plt.savefig('E:/out/{}.png'.format(str(i).zfill(3)), bbox_inches='tight')
        i += 1
        plt.close(fig)
